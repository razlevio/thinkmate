	...
	return (
		<div className="flex flex-col items-center justify-center px-6">
			...
			<div className="mt-12 grid max-w-6xl grid-cols-1 gap-4 md:grid-cols-3">
				{messages[1]?.content && (
					messages[1]?.content.substring(messages[1]?.content.indexOf("1") + 3).split(/2\.|3\./).map((generatedIdea) => {
						const ideaName = generatedIdea.split(":")[0]?.trim()
						const ideaText = generatedIdea.split(":")[1]?.trim()
						return (
							<div
								key={generatedIdea}
